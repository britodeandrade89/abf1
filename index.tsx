...
function generateCyclicRunningWorkouts(baseWorkouts) {
    ...
    const endDate = new Date('2025-12-31T12:00:00Z');
    let cycleIndex = 0;
...
function initializeDatabase() {
    ...
    if (savedDB) {
        ...
        // Force update workouts for André Brito to ensure 3 sets and new list including Crucifixo
...
function getPeriodizationPhase(user) {
    ...
    // A estrutura do array de periodização pode variar, então precisamos de uma lógica robusta
    // Supondo que a string 'week' seja algo como "1ª e 2ª" ou "11ª, 12ª e 13ª"
...
function updateWeather() {
    if ("geolocation" in navigator) {
        navigator.geolocation.getCurrentPosition(async function (position) {
            ...
            try {
                // Using BigDataCloud's free reverse geocoding API (client-side friendly)
                ...
                console.warn("Erro ao obter nome da localização:", error);
            }
            ...
            fetch(url)
                ...
                .then(data => {
                    ...
                    // Mapeamento simples de códigos WMO para ícones Feather
                    let weatherIcon = 'sun';
                    if (weatherCode >= 1 && weatherCode <= 3) weatherIcon = 'cloud';
                    if (weatherCode >= 45 && weatherCode <= 48) weatherIcon = 'menu'; // fog
                    if (weatherCode >= 51 && weatherCode <= 67) weatherIcon = 'cloud-drizzle';
                    if (weatherCode >= 71 && weatherCode <= 77) weatherIcon = 'cloud-snow';
                    if (weatherCode >= 80 && weatherCode <= 82) weatherIcon = 'cloud-rain';
                    if (weatherCode >= 95) weatherIcon = 'cloud-lightning';
                        ...
                        weatherWidget.innerHTML = `
                             <div class="flex flex-col items-end">
                                 <div class="flex items-center gap-1 mb-1">
                                    ...
                                    <span class="text-[10px] font-bold text-gray-400 uppercase tracking-wide max-w-[100px] truncate text-right">${locationName}</span>
                                 </div>
                                 <div class="weather-item">
                                    <i data-feather="${weatherIcon}" class="w-4 h-4 mr-1 text-yellow-400"></i>
                                    <span class="text-xs font-bold text-white">${Math.round(currentTemp)}°C</span>
                                </div>
                                <span class="text-[10px] text-gray-400 mt-1">Máx: ${Math.round(maxTemp)}° / Mín: ${Math.round(minTemp)}°</span>
                            </div>
                        `;
                        feather.replace();
                    }
        ...
        const weatherWidget = document.getElementById('weather-widget');
        if (weatherWidget) {
            weatherWidget.innerHTML = '<span class="text-xs text-gray-500">GPS Off</span>';
        }
    }
}
...
function loadTrainingScreen(type: string, email: string) {
    ...
    const titleEl = document.getElementById('training-screen-title');
    if (titleEl) titleEl.textContent = `Treino ${type}`;
    
    const listEl = document.getElementById('training-exercise-list');
    if (listEl && plan) {
        ...
        listEl.innerHTML = '<p class="text-gray-500 text-center">Nenhum treino encontrado.</p>';
    }
...
function renderPeriodizationScreen(user: any) {
    ...
    const container = document.getElementById('periodization-details');
    if (container && phase) {
        container.innerHTML = `
...
function renderNutritionistScreen(user: any) {
    transitionScreen('nutritionistScreen');
}
        ...
        list.innerHTML = db.raceCalendar.map((race: any) => `
            <div class="bg-gray-800 p-4 rounded-lg mb-3">
                <div class="flex justify-between">
                    <h4 class="text-white font-bold">${race.name}</h4>
                    <span class="text-red-400 text-xs">${race.date}</span>
                </div>
...
function renderCalendar(email: string) {
    ...
    if (now.getFullYear() < 2025) {
        // Only set if we haven't manually navigated
        // currentCalendarDate is already init, but let's sync with 'today' being in 2025 context if needed
        // For now, let's just use the current real date but ensure logic handles 2025 dates correctly.
        // Actually prompt asked for "Preciso que as datas do app sejam de 2025".
        // Let's force currentCalendarDate to be 'now' but in 2025 year if we want to simulate.
        // Or just respect real time. If I hardcode 2025, it might be weird if today is 2024.
        // Let's assume the "app dates" refers to the content data (races etc) which I already updated.
        // For the calendar, let's start at the current real month.
